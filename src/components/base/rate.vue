<template>
    ...
    <template v-for="n in length">
    	<button type="button" :key="n" :class="{'rate-star':true,'filled': n <= rate , 'hover':n <= over,'disabled':disabled}"  @mouseover="onOver(n)" @mouseout="onOut(n)" @click="setRate(n)" :disabled="disabled">
...
.rate-star.hover{color: #efc20f;}
.rate-star.filled{color: #efc20f;}
.rate-star:hover,
.rate-star:focus{text-decoration: none;}

.rate.has-error .rate-star{color: #f37a77;}
.rate.has-error .rate-star.hover{color: #efc20f;}
.rate.has-error .rate-star.filled{color: #efc20f;}
.rate-star[disabled]{
  color: #CCC;
  opacity: 0.8;
}
